...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger = None, fail_case = False):
    ...
    with torch.no_grad():
        ...
        if fail_case:
            ...
            draw_confusion_matrix(test_pred, test_label, os.path.join(save_path, f'confusion_matrix_{str(epoch).zfill(4)}.png'), labels_num=config.model.cls_dim)
