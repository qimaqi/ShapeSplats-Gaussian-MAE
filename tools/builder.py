...
def resume_model(base_model, args, logger = None, strict_load=True):
    ...
    # print("base_ckpt", base_ckpt.keys())
    missing, unexpected = base_model.load_state_dict(base_ckpt, strict = strict_load)
    # print("base_model keys", base_model.state_dict().keys())
    print_log(f'[RESUME INFO] missing keys: {missing}, unexpected{unexpected}', logger = logger)
...
def load_model(base_model, ckpt_path, logger = None, strict_load =True):
    if not os.path.exists(ckpt_path):
    ...
    base_model.load_state_dict(base_ckpt, strict = strict_load)
