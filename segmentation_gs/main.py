...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=224, help='batch Size during training')
    ...
    parser.add_argument('--partanno_root', type=str, default='../data/shapenetcore_partanno_segmentation_benchmark_v0_normal/', help='data root')
    parser.add_argument('--gs_root', type=str, default='../data/gs_data/', help='data root')
    parser.add_argument('--pc_to_gs_map', type=str, default='./pc_to_gs_map.json', help='data root')
    ...
    parser.add_argument('--soft_knn', action='store_true', default=False, help='use soft knn in Encoder')
...
def main(args):
    ...
    if args.use_wandb:
        wandb.init(project='Gaussian-MAE', config=args, name=args.log_dir.split('/')[-1])

    partanno_root = args.partanno_root
    gs_root = args.gs_root

    # args.attribute = literal_eval(args.attribute)
    print("========args.attribute=========", args.attribute)


    TRAIN_DATASET = PartNormalGSDataset(partanno_root=partanno_root, gs_root=gs_root, npoints=args.npoint, split='trainval', normal_channel=args.normal, norm_attribute=args.norm_attribute, pc_to_gs_map=args.pc_to_gs_map)
    trainDataLoader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=args.batch_size, shuffle=True, num_workers=4, drop_last=True)
    TEST_DATASET = PartNormalGSDataset(partanno_root=partanno_root, gs_root=gs_root, npoints=args.npoint, split='test', normal_channel=args.normal, norm_attribute=args.norm_attribute, pc_to_gs_map=args.pc_to_gs_map)
    ...
    if args.ckpts is not None and len(args.ckpts) > 0:
        print("len(args.ckpts)", len(args.ckpts))
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        num_iter = 0
        '''learning one epoch'''
        for i, (gs_data, label, pc_xyz, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            num_iter += 1
            gs_data = gs_data.data.numpy()
            pc_xyz = pc_xyz.data.numpy()
            # TODO 1: we do not augment points place, we argument the gs_data
            gs_data[:, :, 0:3], batch_scale = provider.random_scale_point_cloud(gs_data[:, :, 0:3], return_scales=True)
            gs_data[:, :, 0:3], shifts = provider.shift_point_cloud(gs_data[:, :, 0:3], return_shifts=True)
            for b in range(gs_data.shape[0]):
                gs_data[b, :, 4:7] = gs_data[b, :, 4:7] * batch_scale[b]   
                pc_xyz[b] = pc_xyz[b] * batch_scale[b]  
                pc_xyz[b] = pc_xyz[b] + shifts[b]

            gs_data = torch.Tensor(gs_data)
            ...
            if num_iter == 1: # use for accumulate gradient
        ...
        with torch.no_grad():
            ...
            for batch_id, (gs_data, label,pc_xyz, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
                ...
                cur_batch_size, NUM_POINT, _ = pc_xyz.size() 
                label, target, gs_data, pc_xyz = label.long().cuda(), target.long().cuda(), gs_data.float().cuda(), pc_xyz.float().cuda()
                gs_data = gs_data.transpose(2, 1)
        ...
        if args.use_wandb:
            wandb.log({"test_accuracy": test_metrics['accuracy'], "class_avg_iou": test_metrics['class_avg_iou'], "inctance_avg_iou": test_metrics['inctance_avg_iou']}, step=epoch)
