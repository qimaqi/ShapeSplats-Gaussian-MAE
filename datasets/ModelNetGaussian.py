    ...
    def __init__(self, config):
        ...
        if self.num_category == 10:
            ...
            if self.subset == 'train':
                self.data_list_file = os.path.join(self.data_root,'modelnet10_train.txt')
            else:
                self.data_list_file = os.path.join(self.data_root, 'modelnet10_test.txt')
 
        else:
            self.catfile = os.path.join(self.data_root, 'modelnet40_shape_names.txt')
            if self.subset == 'train':
                self.data_list_file = os.path.join(self.data_root,  'modelnet40_train.txt')
            else:
                self.data_list_file = os.path.join(self.data_root,  'modelnet40_test.txt')
        ...
        for line in lines:
            ...
            model_id = line
            file_path = os.path.join(self.gs_path, taxonomy_id, self.subset, line, 'point_cloud.ply' )
        ...
        print_log(f'[DATASET] {len(self.file_list)} instances were loaded', logger = 'ModelNet Gaussian')
